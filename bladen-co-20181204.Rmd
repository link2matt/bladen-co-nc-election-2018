...
Bladen County, North Carolina may have been a target of a campaign of electoral fraud, acoording to media reports <https://www.nytimes.com/2018/12/04/us/politics/north-carolina-election-fraud.html>. 
Dr. Michael Bitzer of Catawba College has identified several unusual trends evident in the race for US Congress in the 9th District <http://www.oldnorthstatepolitics.com/2018/12/NCs-9th-competitive-and-contested-CD.html>.  Dr. Bitzer reports unsually low rates of acceptance for absentee mail-in ballots for voters in Bladen County who voted in the US Congres 9th District race. When this report was published, Republican Mark Harris led his opponent Democrat Dan McCready by less than 1,000 votes in this compettive contest. 
...
## Methods
Ths analysis was generated using the R programming language using data from the North Carolina State Board of Elections (NCSBE).  Data on election results and voters are availabe online for download <https://dl.ncsbe.gov/index.html>.
...
NCSBE makes availabe an 800MB file with details on all votes cast in this election, including all requests for absentee ballots.  Because of the size of this data file and its' public availabiilty from NCSBE, this file isn't stored in the github repository for this analysis.  The file absentee_20181106.csv was downloaded for this report on December 4, 2018.
...
The analysis relies on the geocoding service available at geocod.io.  Addresses of voters who attempted to vote with mail-in ballots were geocoded, generating latitude and longtitude locations for the voters.  For this analysis, a copy of the ballots data was read from a file downloaded from geocodd.io.
...
Below is the map of all mail-in ballots for the 9th Congressional District, categorized by race and whether the ballot was accepted. The larger gray cirles are areas where press reports indicate that voters may have been targeted as part of a campaign of voter fraud. 
```{r}
m <- leaflet(Mail9thBladen) %>% 
  ...
  addCircles(suspectAreas$Longitude, suspectAreas$Latitude,
             radius = 2000, color = "#666666", stroke = T, fillOpacity = 0.1
  ) %>% 
...
This R markdown document and some data files used in this analysis are available on GitHub under the link2matt account <https://github.com/link2matt?tab=repositories>